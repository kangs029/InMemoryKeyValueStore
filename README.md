...
1. **Set(key, field, value string)** 
- Should insert a field-value pair to the record associated with key. If the field in the record already exists, replace the existing value with the specified value. If record doesn't exist, create a new one.
2. **Get(key, field string) ->string** 
- Should return the value contained within field of record associated with key. If record or field doesn't exist, should return nil
3. **Delete(key, field string) ->bool** 
...
- The same as Delete, but with timestamp of the operation specified. Should return true if the field existed and was successfully deleted and false if the key didn't exist.
...
## Level 4
1.  Implement a restore functionality:  undo and redo functions.